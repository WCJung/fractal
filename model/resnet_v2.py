...
# @partial(jax.jit, device=jax.devices())
@partial(jax.pmap, axis_name='batch', in_axes=(None, 0, 0, None), out_axes=(None, None))
def update_fn(variables, x, y, lr):
    (loss, (logits, variables)), grads = jax.value_and_grad(loss_fn, has_aux=True)(variables, x, y)
    # jax.debug.print("G={g}", g=jax.tree_map(jnp.shape, grads))
...
def train_and_validate_oneEpoch(variables, batches, tbatches, lr):
    ...
    jax.debug.print("P={p}", p=jax.tree_map(jnp.shape, variables['params']))
    jax.debug.print("G={g}", g=jax.tree_map(jnp.shape, grads))
