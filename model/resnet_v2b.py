...
def update_fn(variables, x, y, lr):
    ...
    return variables, (loss, logits, grads)
...
def train_on_the_track(variables, batches, tbatches, hparams, epochs, desc=None):
    ...
    variables = jax.tree_map(lambda v: v.reshape(((4, v.shape[0]//4) + v.shape[1:])), variables)
    lr = jax.tree_map(lambda lr: lr.reshape(((4, lr.shape[0]//4) + lr.shape[1:])), lr)

    for _ in tqdm(range(epochs), total=epochs, desc=desc, leave=False):
        variables, loss, acc, tloss, tacc = train_and_validate_oneEpoch(variables, batches, tbatches, lr)
        
        # print(f"Epoch{_} loss={loss.shape} acc={acc.shape}")
        loss = loss.reshape((loss.shape[0] * loss.shape[1]))
        acc = acc.reshape((acc.shape[0] * acc.shape[1]))
        tloss = tloss.reshape((tloss.shape[0] * tloss.shape[1]))
        tacc = tacc.reshape((tacc.shape[0] * tacc.shape[1]))
...
def train_batch(variables, x, y, lr):
    variables, (loss, logits, grads) = update_fn(variables, x, y, lr)
    acc = (logits.argmax(axis=-1)==y).mean(axis=-1)
    return variables, loss, acc, grads
    
# validating
# pmapped_loss_fn = jax.pmap(loss_fn, axis_name='batch', in_axes=(None, 0, 0, None))
@partial(jax.pmap, in_axes=(0, None, None))             # variables/4
def evaluate_batch(variables, x, y):
    vmapped_loss_fn = jax.vmap(loss_fn, in_axes=(0, None, None, None))
    loss, (logits, _) = vmapped_loss_fn(variables, x, y, False)
    acc = (logits.argmax(axis=-1)==y).mean(axis=-1)
    return loss, acc
...
def train_and_validate_oneEpoch(variables, batches, tbatches, lr):
    ...
    for batch in batches.as_numpy_iterator():
        x = batch['image']
        y = batch['label']
        variables, loss, acc, grads = train_batch(variables, x, y, lr)    
        
    for tbatch in tbatches.as_numpy_iterator():
        tx = tbatch['image']
        ty = tbatch['label']
        tloss, tacc = evaluate_batch(variables, tx, ty)
        
    return variables, loss, acc, tloss, tacc

def means_device(archive, pmap_dim=0):
    '''[np.array(pmap, vmap)]*epochs -> [np.array(vmap)]*epochs'''
    # return [arc.mean(axis=0) for arc in archive]
    return list(map(lambda arc: arc.mean(axis=pmap_dim), archive))
...
def initialize(module, rng, x):
    ...
    variables['params']['Dense_0']['kernel'] = jax.nn.initializers.xavier_normal()(jax.random.PRNGKey(1), (50176, 10)) * 1e-6  # 64 * 28**2
